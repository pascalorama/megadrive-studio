...
void vdp_data_w(uint16 data)
{
    ...
    switch(code & 0x0F)
    {
        case 0x01: /* VRAM */
            ...
            {
				if((addr & sat_addr_mask)==4 && data!=0 )
				{
					int i=data; 
				}
    ...
    {	int length = (reg[20] << 8 | reg[19]) & 0xFFFF;
		...
		level_error(1,"pc:%.8x:VRAM Fill %d bytes with value %.2x into 0x%.5x",m68k_get_reg(NULL, M68K_REG_PC),length,((data >> 8) & 0xFF),((addr- reg[15])- 0xFFFF));
...
void vdp_reg_w(uint8 r, uint8 d)
{
    switch(r)
    {
		...
		/* Provoque un breakpoint */
		case 29:
		{
			if(d==0)
			{	m_CallBacks->m_fctSetBreakPoint(m68k_get_reg(NULL, M68K_REG_PC));
				m_GotBreakpoint=1;
			}
...
void dma_copy(void)
{
	...
	level_error(1,"pc:%.8x:VRAM Copy src[0x%.4x]->dst[0x%.4x] length=0x%.5x",m68k_get_reg(NULL, M68K_REG_PC),source,addr,length);
...
*/
	...
	{
		case 0x1:
		{	level_error(1,"pc:%.8x:68k -> VRAM of %.5d words from  0x%.8x to 0x%.4x (VRAM Access)",m68k_get_reg(NULL, M68K_REG_PC),length,source,addr);
		}break;

		case 0x3:
		{	level_error(1,"pc:%.8x:68k -> VRAM of %.5d words from  0x%.8x to 0x%.4x (CRAM Access)",m68k_get_reg(NULL, M68K_REG_PC),length,source,addr);
		}break;

		case 0x5:
		{	level_error(1,"pc:%.8x:68k -> VRAM of %.5d words from  0x%.8x to 0x%.4x (VSRAM Access)",m68k_get_reg(NULL, M68K_REG_PC),length,source,addr);
		}break;
	}	
