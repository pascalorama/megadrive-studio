...
uint8 io_reg[0x10];
uint8 region_code = REGION_EUROPE;
  ...
  switch (offset)
  {
    case 0x01: /* Port A Data */
	  ...
	  {		port[offset-1].data_w(value);
			level_error(2,"IO Write PC 0x%.8x Port A DATA 0x%.2x ",m68k_get_reg(NULL,M68K_REG_PC),value);
			return;
	  }
    ...
    case 0x02: /* Port B Data */
		...
		if(port[offset-1].data_w) 
		{	port[offset-1].data_w(value);
			level_error(2,"IO Write PC 0x%.8x Port B DATA 0x%.2x ",m68k_get_reg(NULL,M68K_REG_PC),value);
			return;
		}
    ...
    case 0x03: /* Port C Data */
		...
		if(port[offset-1].data_w) 
		{	port[offset-1].data_w(value);
			level_error(2,"IO Write PC 0x%.8x Port C DATA 0x%.2x ",m68k_get_reg(NULL,M68K_REG_PC),value);
			return;
		}
    case 0x05:      /* Port B Ctrl */
	  ...
	  {	// rajout pbo
		...
		level_error(2,"IO Write PC 0x%.8x Port B CTRL 0x%.2x ",m68k_get_reg(NULL,M68K_REG_PC),value);
    ...
    case 0x04:      /* Port A Ctrl */
		...
		level_error(2,"IO Write PC 0x%.8x Port A CTRL 0x%.2x ",m68k_get_reg(NULL,M68K_REG_PC),value);
    ...
    case 0x06:      /* Port C Ctrl */
	  ...
	  level_error(2,"IO Write PC 0x%.8x Port C CTRL 0x%.2x ",m68k_get_reg(NULL,M68K_REG_PC),value);
...
uint32 io_read(uint32 offset)
{
  switch(offset)
  {
    case 0x00: /* Version register */
    {
      ...
      uint8 gen_ver = 0x00; /* hardware version */
