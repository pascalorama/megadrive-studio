...
#define get_ea_mode_str_8_symbol(instruction,type_symbol) get_ea_mode_str_symbol(instruction, 0,type_symbol)
#define get_ea_mode_str_16_symbol(instruction,type_symbol) get_ea_mode_str_symbol(instruction, 1,type_symbol)
#define get_ea_mode_str_32_symbol(instruction,type_symbol) get_ea_mode_str_symbol(instruction, 2,type_symbol)
...
static char* get_ea_mode_str(uint instruction, uint size)
{
	...
	switch(instruction & 0x3f)
	{
		...
		/* absolute short address */
			uint temp=read_imm_16();
			char* c=m_CallBacks->m_fctGetSymbolName(-1,temp);
			if(c==NULL)
				sprintf(mode, "$%x.w",temp);
			else
				sprintf(mode,"%s",c);			
			}
		break;
		...
		/* absolute long address */
			uint temp=read_imm_32();
			char* c=m_CallBacks->m_fctGetSymbolName(-1,temp);
			if(c==NULL)
				sprintf(mode, "$%x.l", temp);
			else
				sprintf(mode,"%s",c);
			}			             
		break;
...
static char* get_ea_mode_str_symbol(uint instruction, uint size,int type_symbol)
{
	static char b1[64];
	static char b2[64];
	static char* mode = b2;
	...
	switch(instruction & 0x3f)
	{
		...
		case 0x38:
			{
		/* absolute short address */
			unsigned short temp=read_imm_16();
			char* c=m_CallBacks->m_fctGetSymbolName(type_symbol,temp);
			if(c==NULL)
				sprintf(mode, "$%x.w",temp);
			else
				sprintf(mode,"%s",c);			
		...
		case 0x39:
			{
		/* absolute long address */
			unsigned long temp=read_imm_32();
			char* c=m_CallBacks->m_fctGetSymbolName(type_symbol,temp);
			if(c==NULL)
				sprintf(mode, "$%x.l", temp);
			else
...
static void d68000_bcc_8(void)
{
	...
	uint temp=temp_pc + make_int_8(g_cpu_ir);
	char* c=m_CallBacks->m_fctGetSymbolName(1,temp);
	if(c==NULL)
		sprintf(g_dasm_str, "b%-2s     %x", g_cc[(g_cpu_ir>>8)&0xf], temp);
	else
		sprintf(g_dasm_str, "b%-2s     %s", g_cc[(g_cpu_ir>>8)&0xf], c);		
...
{
	uint temp_pc = g_cpu_pc;
	uint temp=temp_pc + make_int_16(read_imm_16());
	char* c=m_CallBacks->m_fctGetSymbolName(1,temp);
	if(c==NULL)
		sprintf(g_dasm_str, "b%-2s     %x", g_cc[(g_cpu_ir>>8)&0xf], temp);
	else
...
static void d68000_bra_8(void)
{
	...
	char* c=m_CallBacks->m_fctGetSymbolName(1,temp);
	if(c==NULL)
		sprintf(g_dasm_str, "bra     %x", temp);		
	else
		sprintf(g_dasm_str, "bra     %s", c);
}
...
static void d68000_dbra(void)
{
	...
	char* c=m_CallBacks->m_fctGetSymbolName(1,temp);
	if(c==NULL)
		sprintf(g_dasm_str, "dbra    D%d, %x", g_cpu_ir & 7, temp);
	else
		sprintf(g_dasm_str, "dbra    D%d, %s", g_cpu_ir & 7, c);	
...
static void d68000_dbcc(void)
{
	...
	char* c=m_CallBacks->m_fctGetSymbolName(1,temp);
	if(c==NULL)
		sprintf(g_dasm_str, "db%-2s    D%d, %x", g_cc[(g_cpu_ir>>8)&0xf], temp);
	else
		sprintf(g_dasm_str, "dbra    D%d, %s", g_cpu_ir & 7, c);	
...
{
	sprintf(g_dasm_str, "jmp     %s", get_ea_mode_str_32_symbol(g_cpu_ir,1));
}

static void d68000_jsr(void)
{
	sprintf(g_dasm_str, "jsr     %s", get_ea_mode_str_32_symbol(g_cpu_ir,1));
