...
//  0xFE, 0xFF
//};
extern uint8 hc_256[171];
extern uint8 hc_320[210];


static __inline void lightgun_reset(int num)
{
  input.analog[num][0] = bitmap.viewport.w >> 1;
  input.analog[num][1] = bitmap.viewport.h >> 1;
}

static __inline void lightgun_update(int num)
{
...
static __inline void mouse_reset()
{
  mouse.State   = 0x60;
  mouse.Counter = 0;
  mouse.Wait = 0;
...
static __inline void gamepad_raz(uint32 i)
{
  gamepad[i].Counter = 0;
  gamepad[i].Delay   = 0;
}

static __inline void gamepad_reset(uint32 i)
{
  gamepad[i].State = 0x40;
  if (input.dev[i] == DEVICE_6BUTTON) gamepad_raz(i);
}

static __inline void gamepad_update(uint32 i)
{
  if (gamepad[i].Delay++ > 25) gamepad_raz(i);
}

static __inline uint32 gamepad_read(uint32 i)
{
  int control;
  int retval = 0x7F;
...
static __inline void gamepad_write(uint32 i, uint32 data)
{
  if (input.dev[i] == DEVICE_6BUTTON)
  {
...
static __inline void teamplayer_reset(uint32 port)
{
  int i;
  int index = 0;
  int pad_input = 0;
...
*/
static __inline uint32 teamplayer_read_device(uint32 port, uint32 index)
{
  int retval = 0x7F;
...
static __inline uint32 teamplayer_read(uint32 port)
{
  int retval = 0x7F;
  int padnum;
...
static __inline void teamplayer_write(uint32 port, uint32 data)
{
 ...
 *
 *****************************************************************************/
static __inline void wayplay_write(uint32 port, uint32 data)
{
  if (port == 0) gamepad_write(input.current, data);
  else input.current = (data >> 4) & 0x07;
}

static __inline uint32 wayplay_read(uint32 port)
{
